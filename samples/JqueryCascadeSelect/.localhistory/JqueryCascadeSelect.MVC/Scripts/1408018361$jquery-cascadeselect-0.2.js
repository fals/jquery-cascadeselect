(function ($) {
    $.fn.cascadeselect = function (options) {
        ...
        var _getSelectOptions = function (select, url) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                      _resetSelect(select, $(select).data("cas-selectText"));
                      ...
                      _resetSelect(select, $(select).data("cas-noneResultText"));
                    }
                }
        ...
        var _populateSelect = function (select) {
            $("select[data-cas-parentselect='" + $(select).attr('id') + "']").each(function (i, e) {
                ...
                if (value != null && value > 0 && value !== 'undefined') {
                  ...
                  _resetSelect(e, $(e).data("cas-selectText"));
                }
            });
        }
        ...
        var _init = function (select) {
            ...
            $(select).attr("data-cas-selectText", defaultOptions.selectText);
            $(select).attr("data-cas-loadingText", defaultOptions.loadingText);
            $(select).attr("data-cas-noneResultText", defaultOptions.noneResultText);
